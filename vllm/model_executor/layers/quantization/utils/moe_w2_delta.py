...
Pieces:
  ...
  - manager thread: between forwards, consumes the last-seen expert flags
    ...
    Passes are EVENT-DRIVEN: the worker opens a manager window after each
    completed forward and closes it before the next one. The manager runs at
    most one pass per signal, rate-limited by VLLM_MOE_W2_DELTA_TICK_MS; a
    wall-clock timeout only provides liveness for configs that never signal.
...
class DeltaTier:
    def __init__(self, n_layers: int, n_experts: int, dev,
        ...
        self._stop = False
        self._thread = None
        # Manager passes may rewrite pool slots that CUDA graphs read without
        # taking the Python tier lock. Keep the whole pass outside the forward
        # window: pause_for_forward() acquires this lock before a target starts;
        # wake() releases it only after every fixed-point/gate replay finishes.
        self._forward_lock = threading.Lock()
        self._forward_paused = False
        ...
        # Slots touched since step_begin(), or by the current eager prefill
        # layer. Never evictable (even in the emergency pass) —
    ...
    def stop(self) -> None:
        """Stop and join the manager before Torch/Python module teardown.

        Python 3.12 + the v0.25 stable-libtorch extension can abort at process
        exit if a daemon manager is still copying tensors while pybind and
        Torch interpreter state are being destroyed. Serving workers normally
        live until process exit, so an atexit-driven explicit join is the only
        reliable ownership boundary.
        """
        self._stop = True
        if self._forward_paused:
            self._forward_paused = False
        ...
        if thread is None or thread is threading.current_thread():
            return
        thread.join(timeout=max(_FALLBACK_S, _TICK_S) + 2.0)
        if thread.is_alive():
            logger.warning("moe_w2 delta manager did not stop before timeout")
        else:
            self._thread = None
    ...
    def _loop(self):
        ...
        while not self._stop:
            ...
            if self._stop:
                break
            # A target/replay sequence holds this lock. If the next target
            # starts before an event-driven pass does, the pass waits; if the
            # pass already started, the target waits for it to finish.
    ...
    def pause_for_forward(self) -> None:
        """Block until any manager pass finishes, then exclude new passes.

        Called on the runner thread before routing marks are cleared and a new
        target forward starts. Idempotence keeps a failed prior forward safe:
        the next call remains paused until a later worker completion releases
        the window.
        """
        if self._forward_paused:
            return
        self._forward_lock.acquire()
        self._forward_paused = True

    def wake(self):
        """Finish the forward window and signal one manager pass."""
        if self._forward_paused:
            self._forward_paused = False
            self._forward_lock.release()
    ...
    def _tick_once(self):
        ...
        with self._snap_lock:
            ...
            # Keep eviction protection bound to this exact snapshot. Other
            # snapshot users may overwrite the shared host buffer after the
            # lock is released.
            seen_set = {tuple(pair) for pair in seen.tolist()}
        if seen.numel() == 0:
            return
    ...
    def _take_slots_batch(
        self,
        k: int,
        emergency: bool = False,
        min_cold: int = 2,
        seen_set: set[tuple[int, int]] | None = None,
    ) -> list[int]:
        ...
        The seen window is a recency heuristic after the target has drained;
        step pins and split-FP4 residency coupling remain hard exclusions.
        ...
        the caller's immutable seen snapshot (or _seen_host for best-effort
        callers without one) and cold >= 2 ticks, so in-flight graph reads
        never hit a rewritten slot.
        ...
        if self._policy == "need":
            ...
            # LRU ticks are int64, but masked candidates need an infinity
            # sentinel below. Promote to float so a saturated LRU pool can
            # exclude in-flight slots instead of raising on the assignment.
            key = tk.double()
        # Hard exclusions: free markers and step-pinned slots (touched by a
        # pass that the current step will replay). The seen window is a soft
        # recency exclusion once the target has drained.
        if seen_set is None:
            active_seen = self._seen_host[lic, eic].to(torch.bool)
        else:
            seen_mask = torch.zeros_like(self._seen_host, dtype=torch.bool)
            if seen_set:
                seen_li, seen_ei = zip(*seen_set)
                seen_mask[list(seen_li), list(seen_ei)] = True
            active_seen = seen_mask[lic, eic]
        hard = li < 0
        if self._step_pins:
            hard[list(self._step_pins)] = True
        blocked = hard | active_seen
        ...
        if self._coupled_fp4 is not None:
            coupled = self._coupled_fp4._mirror[lic, eic] >= 0
            hard |= coupled
            blocked |= coupled
        ...
        # bound so they can never churn the hot set). Passes 2-3 (emergency):
        # synchronous callers relax first the coldness bound, then the seen
        # window, rather than leave a missing expert UNRESTORED. MTP verify
        # can mark nearly the whole pool in one step, so keeping the seen
        # window hard in pass 3 starves the restore path.
    ...
    def _take_slot(self, seen_set=None):
        ...
        slots = self._take_slots_batch(1, seen_set=seen_set)
    ...
    def step_begin(self) -> None:
        ...
        prefill: before the first ensure_resident). Slots touched after this call are
        pinned against eviction until the next step_begin — the fixed-point
        replay's passes must never cannibalize each other's fetches. The worker
        signals the manager only after every replay has completed."""
        with self._lock:
            self._step_pins.clear()

    def routing_step_begin(self) -> None:
        """Discard routing marks left by warmup or the previous target step."""
        main = torch.cuda.current_stream(self.dev)
        with self._snap_lock:
            self.seen.zero_()
            # The manager snapshots on this side stream without otherwise
            # waiting for the runner. Order future snapshots after the zero.
            with torch.cuda.stream(self._stream):
                self._stream.wait_stream(main)

    def step_end(self) -> None:
        """Close the target/replay routing window before managers resume."""
        self.routing_step_begin()
    ...
    def force_promote(self, layers=None, max_promote=None, pin: bool = True) -> int:
        ...
        Args:
            ...
            pin: pin touched slots when another replay will read this step.
                Pass False for the final fetch when no replay follows.
        Returns:
        ...
        if seen.numel() == 0:
            return 0
        # Eviction must use the immutable routing snapshot captured above,
        # not the shared host buffer that another caller can overwrite.
        seen_set = {tuple(pair) for pair in seen.tolist()}
        ...
        with self._lock:
            ...
            slots = self._take_slots_batch(
                len(cand), emergency=True, seen_set=seen_set
            )
        ...
        if len(plan) < len(cand):
            if not pin:
                # No reader follows this call. The next routed step refetches
                # under its own mandatory pass, so this is a deferred warm-up.
                self._kpi_deferred += len(cand) - len(plan)
            else:
                # A replay will re-read this step, so any slotless candidate
                # remains a correctness-visible zeroed contribution.
                ...
                logger.warning(
                    ...
                    "contributions [pool %d, pinned %d, free %d, wanted %d]",
                    self._tag, len(cand) - len(plan), self.n_slots,
                    len(self._step_pins), len(self._free), len(cand))
        if _TRACE >= 2:
    ...
    def ensure_resident(self, layer_key: int, ids: torch.Tensor) -> int:
        ...
        ids = ids.unique().long()
        ids_cpu = ids.cpu()
        layer_seen_set = {(layer_key, int(e)) for e in ids_cpu}
        mark_seen(self.seen[layer_key], ids.to(self.dev))
        # Wait for prior-layer work before selecting eviction victims. Keep
        # the device `seen` aggregate intact for telemetry, but protect only
        # this layer's experts during the layer-at-a-time eager prefill scan;
        # retaining prior layers in the eviction mask union-saturates the pool.
        main = torch.cuda.current_stream(self.dev)
        with self._snap_lock:
            with torch.cuda.stream(self._stream):
                self._stream.wait_stream(main)
            ...
            ev.synchronize()
        with self._lock:
            # Each eager layer starts after the prior layer drained. Scope
            # pins to this layer so the manager cannot recycle a current
            # hit or freshly loaded slot before its GEMMs consume it.
            self._step_pins.clear()
            ...
            if bool(hit.any()):
                ...
                self._owner_tick[slots[hit]] = self._tick
                self._step_pins.update(int(s) for s in slots[hit].tolist())
            ...
            slots = self._take_slots_batch(
                len(cand), emergency=True, seen_set=layer_seen_set
            )
            plan = [((li, ei), slot) for (li, ei), slot in zip(cand, slots)]
            if not plan:
                return 0
...
def mark_seen(seen_row, ids, token_valid=None):
    ...
    cheap binary flag. ``token_valid`` masks padded token rows without changing
    the captured shape."""
    if token_valid is not None:
        if ids.ndim != 2 or token_valid.ndim != 1:
            raise RuntimeError(
                "masked moe_w2 seen recording requires [T, K] ids and [T] validity"
            )
        if ids.shape[0] != token_valid.shape[0]:
            raise RuntimeError(
                "moe_w2 token-validity length does not match routed token rows"
            )
        updates = token_valid[:, None].expand_as(ids).reshape(-1).to(seen_row.dtype)
        ids = ids.reshape(-1)
    else:
        updates = None
    if _COUNT:
        if updates is None:
            updates = torch.ones_like(ids, dtype=seen_row.dtype)
        seen_row.index_add_(0, ids, updates)
    else:
        if updates is None:
            seen_row.index_fill_(0, ids, 1)
        else:
            seen_row.scatter_reduce_(0, ids, updates,
...
def begin_target_step() -> None:
    """Exclude manager rewrites, then clear stale marks before the target."""
    for tier in (_BASE_TIER, _TIER):
        if tier is not None:
            tier.pause_for_forward()
            tier.routing_step_begin()


def begin_replay_step() -> None:
    """Open both tiers' pin scopes after the target forward, before replay."""
    for tier in (_BASE_TIER, _TIER):
        if tier is not None:
            tier.step_begin()


def finish_forward_step() -> None:
    """Close routing windows, then resume managers between forwards."""
    for tier in (_BASE_TIER, _TIER):
        if tier is not None:
            tier.step_end()
    wake_all()
...
def _arm_split_coupling() -> None:
    """Couple split-FP4 refinement residency to the base tier.

    Fresh requant builds create the FP4 tier before ``_finish_layer`` creates
    the base tier, while pack-hit restores create them in the opposite order.
    Invoke this after either singleton lookup so coupling cannot depend on
    which path constructed its tier first.
    """
    if not split_enabled() or _BASE_TIER is None or _TIER is None:
        return
    if _BASE_TIER._coupled_fp4 is _TIER:
        return
    _BASE_TIER._coupled_fp4 = _TIER
    logger.info(
        "moe_w2 delta: split-FP4 residency coupling armed "
...
def wake_all() -> None:
    """Post-forward broadcast: release and nudge each live tier manager."""
    t = _BASE_TIER
    if t is not None:
        t.wake()
    t = _TIER
...
def shutdown_all() -> None:
    """Join every distinct tier manager while Torch is still fully alive."""
    stopped: set[int] = set()
    for tier in (_BASE_TIER, _TIER):
        if tier is not None and id(tier) not in stopped:
            stopped.add(id(tier))
            tier.stop()
