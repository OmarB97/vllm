...
def _ckpt_id() -> str:
    """Cheap identity of the checkpoint the planes derive from.

    Hash the canonical model path and index contents, then the name, size and
    nanosecond mtime of every referenced shard. This catches an in-place
    checkpoint replacement without hashing a DS4-sized checkpoint at boot.
    """
    ...
    if os.path.exists(idx):
        ...
        with suppress(AttributeError, TypeError, ValueError,
            ...
            shards.update(json.loads(raw).get("weight_map", {}).values())
    if not shards and os.path.isdir(model):
        shards.update(name for name in os.listdir(model)
...
def cache_identity() -> dict:
    """Public deterministic identity shared by every derived W2 cache."""
    return _meta()
