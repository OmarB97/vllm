...
def record(layer_key: int, x: torch.Tensor, topk_ids: torch.Tensor,
    ...
    if route_log is not None:
        ...
        _hit[0] += (m0 & valid_routes).sum()
        _tot[0] += token_valid.sum() * k_true
    ...
    if layer_key > 0 and layer_key in _gate_w:
        ...
        _hit[1] += (m1 & valid_routes).sum()
        _tot[1] += token_valid.sum() * k_true
    ...
    if w is not None:
        ...
        pred_ids = torch.where(token_valid[:, None], pred_ids,
                               torch.full_like(pred_ids, -1))
