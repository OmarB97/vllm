...
def initialize_model_parallel(
    ...
    tensor_axis = ParallelAxis(
        ...
        rank=_TP.rank_in_group,
        group=ParallelGroupType.TENSOR,
    )
    tensor_plan = LayerParallelPlan(input=tensor_axis, output=tensor_axis)
    overrides = None
    if _ATTENTION_TP is not None:
        attention_axis = ParallelAxis(
            world_size=_ATTENTION_TP.world_size,
            rank=_ATTENTION_TP.rank_in_group,
            group=ParallelGroupType.ATTENTION_TENSOR,
        )
        overrides = {
            LayerType.ATTENTION: LayerParallelPlan(
                input=attention_axis,
                output=tensor_axis,
            )
        }
    init_layer_parallel_config(tensor_plan, overrides)
